...
const RANK_FILE = path.join(process.cwd(), 'ranking.json');
...
async function recordMatch(winner, loser) {
  ...
  if (!ranking.players[winner.id]) ranking.players[winner.id] = createPlayer(winner.username);
  if (!ranking.players[loser.id]) ranking.players[loser.id] = createPlayer(loser.username);
...
async function tradeItem(userId, item) {
  ...
  if (!player) return { success: false, message: 'Perfil não encontrado.' };

  if (item === 'sukuna') {
    if ((player.items?.sukuna ?? 0) < 2) {
    ...
    return { success: true, reward: { roleId: '1463413152824819753' } };
  }
  ...
  if (item === 'gokumonkyo') {
    ...
    return { success: true, reward: { roleId: '1463413249734086860' } };
  }
